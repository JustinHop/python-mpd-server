    ...
    def __parseArg(self, args):
        ...
        if len(args) > len(self.formatArg):
            raise CommandArgumentException(
                "Too much arguments: %s command arguments should be %s instead of %s" %
                (self.__class__, self.formatArg, args))
        try:
            d = dict()
            for i in range(0, len(self.formatArg)):
                if Opt in self.formatArg[i][1].__bases__:
                    try:
                        d.update(
                            {self.formatArg[i][0]: self.formatArg[i][1](args[i])})
                    except IndexError:
                        pass
                else:
                    d.update(
            ...
            raise CommandArgumentException(
                "Not enough arguments: %s command arguments should be %s instead of %s" %
                (self.__class__, self.formatArg, args))
        except ValueError as e:
            raise CommandArgumentException(
                "Wrong argument type: %s command arguments should be %s instead of %s (%s)" %
                (self.__class__, self.formatArg, args, e))
        return d

    def handle_args(self, **kwargs):
        """ Override this method to treat commands arguments."""
        logger.debug("Parsing arguments %s in %s" %
                     (str(kwargs), str(self.__class__)))
...
class MpdPlaylistSong(object):
    ...
    def toMpdMsg(self):
        if self.playlistPosition is None:
            logger.warning(
                "MpdPlaylistSong.playlistposition attribute is not set.")
...
class MpdPlaylist(object):
    ...
    def handlePlaylist(self):
        ...
        raise NotImplementedError(
            "you should implement MpdPlaylist.handlePlaylist method (or use MpdPlaylistDummy) ")
    ...
    def songIdToPosition(self, id):
        ...
        raise NotImplementedError(
            "you should implement MpdPlaylist.songIdToPosition method")
    ...
    def move(self, fromPostion, toPosition):
        ...
        raise NotImplementedError(
            "you should implement MpdPlaylist.move method")
    ...
    def delete(self, position):
        ...
        raise NotImplementedError(
            "you should implement MpdPlaylist.delete method")
