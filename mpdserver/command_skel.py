...
class CurrentSong(CommandSongs):
    pass
...
class Status(CommandItems):
    ...
    def helper_status_common(self, volume=0, repeat=0, random=0, xfade=0):
        ...
        return [('volume', volume),  # (0-100)
                ...
                ('playlistlength', self.playlist.length()),
...
class NotCommands(CommandItems):
    pass  # Not used by gmpc
    # def items(self):
   #     return [('command','tagtypes'),
   #             ('command','lsinfo')]


class Commands(CommandItems):
    pass  # Not used by gmpc
...
class PlChanges(CommandSongs):
    ...
    def songs(self):
        return self.playlist.generateMpdPlaylistDiff(
            self.args['playlistVersion'])
